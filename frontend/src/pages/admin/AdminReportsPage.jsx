    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-white/10 pb-5">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white mt-1 flex items-center gap-2.5 font-heading">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <Link
            ...
            className="px-3 py-2 rounded-xl bg-gradient-to-r from-teal-500/20 to-brand-cyan/20 hover:from-teal-500/30 hover:to-brand-cyan/30 border border-brand-cyan/40 text-brand-cyan hover:text-white transition-all text-xs flex items-center gap-1.5 font-extrabold shadow-sm"
          ...
          <button
            ...
            disabled={!reportData}
            className="btn-primary text-xs py-2 px-3.5 font-bold rounded-xl flex items-center gap-2 shadow-cyan-glow cursor-pointer"
          ...
          <button
            ...
            disabled={!reportData}
            className="py-2 px-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 hover:text-white text-xs font-semibold flex items-center gap-1.5 cursor-pointer"
      ...
      {timeRange === 'custom' && (
        <form onSubmit={handleApplyCustomDate} className="p-4 rounded-2xl glass-card border border-brand-cyan/30 flex flex-wrap items-center gap-4 bg-brand-cyan/[0.03]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-white">Custom Period:</span>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="px-4 py-1.5 rounded-xl bg-brand-cyan hover:bg-brand-cyan/80 text-slate-950 text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
          >
      ...
      {exportNotice && (
        <div className="p-3 rounded-xl bg-emerald-500/20 border border-emerald-400/40 text-emerald-200 text-xs text-center flex items-center justify-center gap-2 animate-in fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-[#0B132B] dark:bg-[#070D18] rounded-2xl p-6 border border-slate-800/90 shadow-2xl relative overflow-hidden flex flex-col justify-between hover:border-slate-700 transition-all">
          {/* Header */}
          ...
          <div className="grid grid-cols-3 gap-2 text-center my-2">
            <div>
              ...
              <div className="text-xs text-slate-400 font-medium mt-1">In Progress</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="text-xs text-slate-400 font-medium mt-1">On Hold</div>
            </div>
          </div>

          {/* Progression Bar */}
          <div className="my-6">
            <div className="border border-emerald-500/40 bg-emerald-950/40 rounded-full py-2.5 px-4 relative overflow-hidden flex items-center justify-center shadow-inner">
              <div 
                className="absolute left-0 top-0 bottom-0 bg-emerald-500/25 rounded-full transition-all duration-700"
                style={{ width: `${Math.min(100, Math.max(0, projects.avg_progression))}%` }}
              />
              <span className="text-xs font-extrabold text-emerald-400 relative z-10 tracking-wide">
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-800/80 text-center">
            <div>
              ...
              <div className="text-[11px] text-slate-400 mt-0.5">Total Company Projects</div>
            </div>
            <div>
              <div className="text-xl font-extrabold text-white font-mono">
                {projects.sales_orders}
              </div>
              <div className="text-[11px] text-slate-400 mt-0.5">Sales Orders</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#0B132B] dark:bg-[#070D18] rounded-2xl p-6 border border-slate-800/90 shadow-2xl relative overflow-hidden flex flex-col justify-between hover:border-slate-700 transition-all">
          ...
          <div className="space-y-4 my-2">
            ...
            <div className="flex items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2 min-w-[110px]">
                <span className="w-6 h-6 rounded-md bg-emerald-950/90 text-emerald-400 border border-emerald-700/50 text-[11px] font-bold flex items-center justify-center font-mono">
                  {billing.fully_paid?.count || 0}
                </span>
                <span className="font-semibold text-slate-200">Fully Paid</span>
              </div>

              <div className="flex-1 mx-2 h-2 rounded-full bg-slate-800/90 relative overflow-hidden">
                <div 
              ...
              <div className="font-mono font-bold text-white text-sm shrink-0 min-w-[90px] text-right">
            ...
            <div className="flex items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2 min-w-[110px]">
                <span className="w-6 h-6 rounded-md bg-amber-950/90 text-amber-400 border border-amber-700/50 text-[11px] font-bold flex items-center justify-center font-mono">
                  {billing.partially_paid?.count || 0}
                </span>
                <span className="font-semibold text-slate-200">Partially Paid</span>
              </div>
            ...
            <div className="flex items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-2 min-w-[110px]">
                <span className="w-6 h-6 rounded-md bg-rose-950/90 text-rose-400 border border-rose-700/50 text-[11px] font-bold flex items-center justify-center font-mono">
                  {billing.outstanding_due?.count || 0}
                </span>
                <span className="font-semibold text-slate-200">Outstanding Due</span>
              </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-800/80">
            <div>
              <div className="text-[11px] text-slate-400">Total Invoiced</div>
            ...
            <div className="text-right">
              <div className="text-[11px] text-slate-400">Outstanding Due</div>
        ...
        <div className="bg-[#0B132B] dark:bg-[#070D18] rounded-2xl p-6 border border-slate-800/90 shadow-2xl relative overflow-hidden flex flex-col justify-between hover:border-slate-700 transition-all">
          ...
          <div className="flex items-center justify-between gap-4 my-2">
            ...
            <div className="flex-1 space-y-3 pl-2">
              <div>
                <div className="text-[11px] text-slate-400 font-medium">Total Revenue Collected</div>
                <div className="flex items-center gap-1.5 font-mono font-black text-emerald-400 text-sm sm:text-base mt-0.5">
              ...
              <div>
                <div className="text-[11px] text-slate-400 font-medium">Logged Expenses &amp; COGS</div>
                <div className="flex items-center gap-1.5 font-mono font-black text-pink-500 text-sm sm:text-base mt-0.5">
      ...
      {/* ========================================================================= */}
      <div className="p-4 rounded-2xl bg-gradient-to-r from-teal-950/60 via-slate-900 to-slate-950 border border-teal-500/30 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl">
        <div className="flex items-center gap-3.5">
          <div className="w-10 h-10 rounded-xl bg-teal-500/20 border border-teal-500/40 flex items-center justify-center text-teal-300 shrink-0 shadow-sm">
        ...
        <Link
          to="/admin/staff-sales"
          className="px-4 py-2 rounded-xl bg-gradient-to-r from-teal-500 to-brand-cyan hover:from-teal-400 hover:to-brand-cyan text-slate-950 font-black text-xs flex items-center gap-2 shadow-md shadow-teal-950/40 shrink-0 transition-all hover:scale-105"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 glass-card rounded-2xl p-6 space-y-4 border border-white/10">
          <h3 className="text-xs uppercase font-extrabold tracking-wider text-brand-cyan border-b border-white/10 pb-2.5 flex items-center justify-between">
          ...
          <div className="space-y-3 pt-1">
            {(reportData?.channel_breakdown || []).map((ch, i) => (
              <div key={i} className="flex items-center justify-between text-xs p-3.5 rounded-xl bg-slate-950/60 border border-white/5 hover:border-white/10 transition-colors">
                <div className="flex items-center gap-3">
                  <span className={`w-3.5 h-3.5 rounded-full ${ch.color || 'bg-brand-cyan'} shadow-sm`} />
                  <div>
                    <div className="font-bold text-white text-sm">{ch.channel}</div>
                    <div className="text-[11px] text-slate-400">{ch.count} transactions ({ch.percent}%)</div>
                  </div>
                </div>
                <div className="text-right">
            ...
            {(reportData?.category_performance || []).map((cat, i) => (
              <div key={i} className="space-y-1.5">
                ...
                <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                  <div 
                    className="h-full bg-gradient-to-r from-brand-cyan to-teal-400 rounded-full transition-all duration-500" 
      ...
      <div className="glass-card rounded-2xl p-6 space-y-4 border border-white/10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/10 pb-3">
          <div>
            <h3 className="text-xs uppercase font-extrabold tracking-wider text-brand-cyan flex items-center gap-2">
              <Receipt className="w-4 h-4 text-brand-cyan" />
              Recent Billing Invoices &amp; Counter Ledger
            </h3>
            <p className="text-[11px] text-slate-400">Authenticated retail POS slips, orders and diagnostic receipts</p>
          </div>
              ...
              {(reportData?.recent_transactions || []).map((t, idx) => (
                <tr key={t.id || idx} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="py-3 px-3">
                    <span className="px-2 py-0.5 rounded bg-white/5 text-[11px] font-mono text-slate-300 border border-white/5">
                      {t.mode || 'POS Counter'}
                    </span>
                  </td>
