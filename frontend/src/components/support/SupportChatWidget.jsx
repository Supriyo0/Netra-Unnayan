...
export const SupportChatWidget = ({ 
  ...
  const containerClasses = isEmbedded
    ? "w-full rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 shadow-xl overflow-hidden flex flex-col h-[600px]"
    : "fixed bottom-5 right-5 z-[999] w-[95vw] sm:w-[420px] max-w-[440px] rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/20 shadow-2xl overflow-hidden flex flex-col h-[580px] sm:h-[640px] max-h-[90vh] animate-slideUp";

  return (
    <div className={containerClasses}>
      
      {/* 1. Header with Direct Contact Shortcuts */}
      <div className="bg-gradient-to-r from-slate-950 via-brand-dark to-slate-900 text-white p-4 shrink-0 border-b border-white/10 space-y-3">
        
        {/* Title Bar */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-2xl bg-brand-cyan/20 border border-brand-cyan/30 flex items-center justify-center text-brand-cyan relative">
              <MessageSquare className="w-5 h-5 stroke-[2.2]" />
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 absolute -top-0.5 -right-0.5 ring-2 ring-slate-950 animate-pulse" />
            </div>
            <div>
              <h3 className="text-sm font-black text-white flex items-center gap-1.5">
                <span>Netra Support &amp; Care</span>
                <span className="px-1.5 py-0.5 rounded-md bg-brand-cyan/20 text-brand-cyan text-[9px] font-mono font-bold">
        ...
        <div className="grid grid-cols-2 gap-2 pt-1">
          <a
            ...
            className="flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-black text-xs transition-all shadow-sm group"
          >
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-cyan-500 hover:bg-cyan-600 text-slate-950 font-black text-xs transition-all shadow-sm group"
          >
            <Mail className="w-4 h-4 stroke-[2.4]" />
      ...
      {/* Mode Switcher Bar */}
      <div className="bg-slate-100 dark:bg-slate-800/80 px-4 py-2 flex items-center justify-between border-b border-slate-200 dark:border-white/5 text-[11px] font-bold">
      ...
      {/* 3. Messages Body */}
      <div className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-slate-50 dark:bg-[#081220]/70 scrollbar-thin">
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-1 px-1 flex items-center gap-1">
                    {isBot && <Bot className="w-3 h-3 text-cyan-500" />}
                    {isAdmin && <ShieldCheck className="w-3 h-3 text-amber-500" />}
                    {isUser && <User className="w-3 h-3 text-emerald-500" />}
                    <span>{msg.sender_name || (isUser ? 'You' : 'Netra Support')}</span>
                  </span>

                  {/* Message Bubble */}
                  <div className={`max-w-[85%] rounded-2xl p-3.5 text-xs shadow-sm space-y-2 ${
                    isUser
                      ? 'bg-gradient-to-r from-brand-cyan to-teal-400 text-slate-950 font-medium rounded-tr-xs'
                      : isAdmin
                      ? 'bg-amber-500/15 border border-amber-500/30 text-slate-900 dark:text-amber-100 rounded-tl-xs'
                      : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 text-slate-800 dark:text-slate-100 rounded-tl-xs'
                  }`}>
                    ...
                    {/* Action Button inside Bot Answer */}
                    {msg.action && (
                      <a
                        href={msg.action.link}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900 text-white dark:bg-white dark:text-slate-950 font-black text-[11px] hover:opacity-90 transition-opacity"
                      >
                ...
                <div className="grid grid-cols-1 gap-1.5">
                  {BOT_TOPICS.map((topic) => (
                    <button
                      ...
                      className="text-left p-2.5 rounded-xl bg-white dark:bg-slate-800/80 hover:bg-cyan-50 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-800 dark:text-slate-200 transition-all flex items-center justify-between gap-2 group cursor-pointer"
                    >
                      <div className="flex items-center gap-2 truncate">
                        <span className="text-base">{topic.icon}</span>
                        <span className="text-xs font-bold truncate group-hover:text-brand-cyan">{topic.title}</span>
                      </div>
      ...
      <div className="p-3 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-white/10 shrink-0 space-y-2">
        ...
        <form onSubmit={handleSendMessage} className="flex items-center gap-2">
          ...
          <button
            ...
            className="btn-primary p-2.5 rounded-xl text-slate-950 font-bold disabled:opacity-40 shadow-cyan-glow shrink-0 transition-transform active:scale-95"
            title="Send Message"
          >
            {sending ? (
        ...
        <div className="flex items-center justify-between text-[10px] text-slate-400 px-1">
          <span>Official Helpline: <strong>+91 9382293614</strong></span>
